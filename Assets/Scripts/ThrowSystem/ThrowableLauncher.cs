...
namespace ThrowingSystem
{
    ...
    public class ThrowableLauncher : Bolt.EntityBehaviour
    {
        ...
        public Direction GetThrowingDirection()
        {
            Direction direction = Direction.Default;

            if (Input.GetAxis(Constants.Input.UpAndDownAxis) > 0.3f || Input.GetButton(Constants.Input.UseItemForward)) direction = Direction.Forward;
            else if (Input.GetAxis(Constants.Input.UpAndDownAxis) < -0.3f || Input.GetButton(Constants.Input.UseItemBackward)) direction = Direction.Backward;
